...
That is the purpose if the processor in InfDB.
...
- The ``building_use_id`` and ``building_use``, ``floor_area`` and ``height`` columns are from the corresponding ``val_*`` columns in the ``citydb.property``.
