...
DROP TABLE IF EXISTS pylovo_input.temp_building_hh_grid;
CREATE TEMP TABLE pylovo_input.temp_building_hh_grid AS
...
SELECT * FROM pylovo_input.temp_building_hh_grid;

-- Step 2: Compute households per building
DROP TABLE IF EXISTS pylovo_input.temp_building_households;
CREATE TEMP TABLE pylovo_input.temp_building_households AS
SELECT building_id,
       GREATEST(ROUND(occupants / durchschnhhgroesse)::int, 1) AS estimated_households
FROM pylovo_input.temp_building_hh_grid;

SELECT * FROM pylovo_input.temp_building_households;

-- Step 3: Update original building table
UPDATE pylovo_input.buildings b
SET households = bh.estimated_households
FROM pylovo_input.temp_building_households bh
...
DROP TABLE IF EXISTS pylovo_input.temp_nearest_grid_households;
CREATE TEMP TABLE pylovo_input.temp_nearest_grid_households AS
SELECT
    b.id AS building_id,
